...
fun FavoriteDetailScreen(
    ...
    PullToRefreshBox(
        ...
        when (val state = uiState) {
            is FavDetailUiState.Loading -> DetailLoadingState()
            is FavDetailUiState.Error -> DetailErrorState(
...
// ── Identical to HomeSuccessState ────────────────────────────────────────────
@Composable
private fun DetailSuccessState(state: FavDetailUiState.Success) {
    val current = state.currentWeather
    val forecast = state.forecast
    val prefs = state.prefs
    val timezoneOffset = current.timezone
    val hourlyItems = remember(forecast.list) { forecast.list.take(8) }
